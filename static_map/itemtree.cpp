...
void ItemArray::removeItem(StructItem& item)
{
    ...
    {
        item.m_item.m_arrayItem.m_prev->m_item.m_arrayItem.m_next
        = item.m_item.m_arrayItem.m_next;
    }
    if (item.m_item.m_arrayItem.m_next)
    {
        item.m_item.m_arrayItem.m_next->m_item.m_arrayItem.m_prev
        = item.m_item.m_arrayItem.m_prev;
    }
    item.m_item.m_arrayItem.initNull();
}

StructItem * ItemArray::getMiddleOf(
                                    StructItem * first,
                                    StructItem * last)
{
...
void ItemArray::swapItems(StructItem * item1, StructItem * item2)
{
    ...
    else
    {
        ...
        std::swap(
                  item1->m_item.m_arrayItem.m_next,
                  item2->m_item.m_arrayItem.m_next);
        //have item1's prev point to item2's prev and vice versa
        std::swap(
                  item1->m_item.m_arrayItem.m_prev,
                  item2->m_item.m_arrayItem.m_prev);
    }
    if (m_first == item1)
    {
...
void ItemTree::recursiveConstruct(
                                  StructItem * left,
                                  StructItem * mid,
                                  StructItem * right,
